...
if(!class_exists('acs_dynamic_module')):

class acs_dynamic_module{
    ...
    function __construct(){
        ...
        add_action('acs/save_post',             array($this, '_save_post'), 20);
        add_action('trashed_post',              array($this, '_trashed_post'));
        add_action('untrashed_post',            array($this, '_untrashed_post'));
        add_filter('acs/get_post_types',        array($this, 'get_post_types'), 10, 2);
    ...
    function _current_screen(){
        ...
        // Single
        if(acs_is_screen($this->post_type)){
        ...
        // List
        }elseif(acs_is_screen("edit-{$this->post_type}")){
    ...
    function bulk_actions($actions){
        ...
        acs_unset($actions, 'edit');
    
        foreach($this->tools as $action){
            
            $action_name = $action === 'php' ? 'PHP' : 'Json';
            $actions["export_{$action}"] = __('Export ', 'acs') . $action_name;
    ...
    function handle_bulk_actions($redirect, $action, $post_ids){
        ...
        $post_ids = acs_maybe_get_REQUEST('post');
        ...
        foreach($this->tools as $tool_action){
            ...
            $keys = implode('+', $keys);
            $url = admin_url("edit.php?post_type=acs-field-group&page=acs-tools&tool={$this->tool}&action={$tool_action}&keys={$keys}");
    ...
    function _post_submitbox_misc_actions($post){
        ...
        $status = $post->post_status === 'publish' || $post->post_status === 'auto-draft' ? __("Active",'acs') : __("Inactive",'acs');
        ...
        foreach($this->tools as $action){
            ...
            $action_name = $action === 'php' ? 'PHP' : 'Json';
            $tools[] = '<a href="' . admin_url("edit.php?post_type=acs-field-group&page=acs-tools&tool={$this->tool}&action={$action}&keys={$name}") . '">' . $action_name . '</a>';
            
        }
        
        if($tools){ ?>
        <div class="misc-pub-section acs-misc-export">
        ...
        <script type="text/javascript">
            (function($) {
                ...
                <?php if($tools){ ?>$('.acs-misc-export').insertAfter('.misc-pub-post-status');<?php } ?>
            })(jQuery);
        </script>
        <?php
    ...
    function display_post_states($post_states, $post){
        ...
        if($post->post_status === 'acs-disabled'){
            $post_states['acs-disabled'] = '<span class="dashicons dashicons-hidden acs-js-tooltip" title="' . _x('Disabled', 'post status', 'acs') . '"></span>';
        }
    ...
    function _edit_row_actions($actions, $post){
        ...
        if(!in_array($post->post_status, array('publish', 'acs-disabled')))
            return $actions;
    
        $post_id = $post->ID;
        $name = $this->get_name($post_id);
        
        acs_unset($actions, 'inline hide-if-no-js');
        ...
        foreach($this->tools as $action){
            ...
            $action_name = $action === 'php' ? 'PHP' : 'Json';
            $actions[$action] = '<a href="' . admin_url("edit.php?post_type=acs-field-group&page=acs-tools&tool={$this->tool}&action={$action}&keys={$name}") . '">' . $action_name . '</a>';
    ...
    function get_fields_labels_recursive(&$array, $field){
        ...
        $label .= !empty($field['label']) ? $field['label'] : '(' . __('no label', 'acs') . ')';
        $label .= $field['required'] ? ' <span class="acs-required">*</span>' : '';
