...
if(!class_exists('acs_dynamic_post_types')):

	class acs_dynamic_post_types extends acs_dynamic_module{
		...
		function initialize(){
			...
			$this->post_type = 'acs-dpt';
			$this->label = 'Post Type Label';
			$this->textdomain = 'ACS Extended: Post Types';

			$this->tool = 'acs_dynamic_post_types_export';
			$this->tools = array('php', 'json');
			$this->columns = array(
				'acs-name'         => __('Name', 'acs'),
				'acs-taxonomies'   => __('Taxonomies', 'acs'),
				'acs-posts'        => __('Posts', 'acs'),
			);
		...
		function actions(){
			...
			// Validate
			add_filter('acs/validate_value/name=acs_dpt_name', array($this, 'validate_name'), 10, 4);
			add_filter('acs/update_value/name=acs_dpt_name',   array($this, 'update_name'), 10, 3);

			// Save
			add_filter('acs/post_type/save_args',              array($this, 'save_args'), 10, 3);
			add_action('acs/post_type/save',                   array($this, 'save'), 10, 3);

			// Import
			add_action('acs/post_type/import_fields',          array($this, 'import_fields'), 10, 3);
			add_action('acs/post_type/import',                 array($this, 'after_import'), 10, 2);
		...
		function get_name($post_id){
			...
			return get_field('acs_dpt_name', $post_id);
		...
		function register_post_type(){
			...
			$capability = acs_get_setting('capability');
		...
		function register_user_post_types(){
			...
			$settings = apply_filters('acs/post_type/prepare_register', acs_get_settings($this->settings));
			...
			foreach($settings as $name => $args){
				...
				// Filters
				$args = apply_filters("acs/post_type/register",                 $args, $name);
				$args = apply_filters("acs/post_type/register/name={$name}",    $args, $name);
		...
		function edit_columns_html($column, $post_id){
			...
			switch($column){
				...
				case 'acs-taxonomies':
					...
					$get_taxonomies = acs_get_array(get_field('taxonomies', $post_id));
					...
					if(!empty($get_taxonomies)){
						...
						if(!empty($taxonomies)){
							...
							$taxonomy_labels = acs_get_taxonomy_labels($taxonomies);
		...
		function admin_config(){
			...
			if(!isset($post_type_obj->acs_admin_ppp))
				return;
			...
			?>
            <script type="text/html" id="tmpl-acs-dpt-title-config">
                <a href="<?php echo admin_url("post.php?post={$post->ID}&action=edit"); ?>" class="page-title-action acs-dpt-admin-config"><span class="dashicons dashicons-admin-generic"></span></a>
            </script>

            <script type="text/javascript">
                (function($){
                    $('.wrap .page-title-action').before($('#tmpl-acs-dpt-title-config').html());
                })(jQuery);
            </script>
			<?php
		...
		function admin_archive_posts($query){
			...
			$admin_order_by = acs_maybe_get($object, 'acs_admin_orderby');
			$admin_order = acs_maybe_get($object, 'acs_admin_order');

			if($admin_order_by && !acs_maybe_get_REQUEST('orderby'))
				$query->set('orderby', $admin_order_by);

			if($admin_order && !acs_maybe_get_REQUEST('order'))
		...
		function admin_archive_ppp($ppp, $post_type){
			...
			$object = get_post_type_object($post_type);
			$admin_ppp = acs_maybe_get($object, 'acs_admin_ppp');
		...
		function front_archive_posts($query){
			...
			$archive_ppp = acs_maybe_get($object, 'acs_archive_ppp');
			$archive_orderby = acs_maybe_get($object, 'acs_archive_orderby');
			$archive_order = acs_maybe_get($object, 'acs_archive_order');
		...
		function front_template($template){
			...
			foreach(get_post_types(array(), 'objects') as $post_type){
				...
				// acs_archive_template
				$acs_archive_template = (isset($post_type->acs_archive_template) && !empty($post_type->acs_archive_template));

				// acs_archive_template
				$acs_single_template = (isset($post_type->acs_single_template) && !empty($post_type->acs_single_template));

				// Global check
				if(!$get_post_type || !$is_query_var || (!$acs_archive_template && !$acs_single_template))
					continue;
				...
				// Archive
				if($rule['has_archive'] && $rule['is_archive'] && $acs_archive_template && ($locate = locate_template(array($post_type->acs_archive_template))))
					return $locate;

				// Single
                elseif($rule['is_single'] && $acs_single_template && ($locate = locate_template(array($post_type->acs_single_template))))
					return $locate;
		...
		function validate_name($valid, $value, $field, $input){
			...
			$exclude = array_merge($exclude, acs_get_setting('reserved_post_types', array()));
			...
			$current_post_id = acs_maybe_get_POST('post_ID');
		...
		function update_name($value, $post_id, $field){
			...
			if($_value !== $value){
				acs_delete_settings("{$this->settings}.{$_value}");
			}
		...
		function save_post($post_id){
			...
			// Filters
			$args = apply_filters("acs/post_type/save_args",               $args, $name, $post_id);
			$args = apply_filters("acs/post_type/save_args/name={$name}",  $args, $name, $post_id);
			$args = apply_filters("acs/post_type/save_args/id={$post_id}", $args, $name, $post_id);

			if($args === false)
				return;

			// Actions
			do_action("acs/post_type/save",               $name, $args, $post_id);
			do_action("acs/post_type/save/name={$name}",  $name, $args, $post_id);
			do_action("acs/post_type/save/id={$post_id}", $name, $args, $post_id);
		...
		function save_args($args, $name, $post_id){
			...
			$name = get_field('acs_dpt_name', $post_id);
			$description = get_field('description', $post_id);
			$hierarchical = get_field('hierarchical', $post_id);
			$supports = get_field('supports', $post_id);
			$taxonomies = acs_get_array(get_field('taxonomies', $post_id));
			...
			// Labels
			$labels = acs_get_array(get_field('labels', $post_id));
			$labels_args = array();
			...
			// Capability
			$capability_type = acs_decode_choices(get_field('capability_type', $post_id), true);
			$capabilities = acs_decode_choices(get_field('capabilities', $post_id));
			$map_meta_cap = get_field('map_meta_cap', $post_id);

			// Archive
			$archive_template = get_field('acs_dpt_archive_template', $post_id);
			$archive_posts_per_page = (int) get_field('acs_dpt_archive_posts_per_page', $post_id);
			$archive_orderby = get_field('acs_dpt_archive_orderby', $post_id);
			$archive_order = get_field('acs_dpt_archive_order', $post_id);
			$has_archive = get_field('has_archive', $post_id);
			$has_archive_slug = get_field('has_archive_slug', $post_id);

			// Single
			$single_template = get_field('acs_dpt_single_template', $post_id);
			...
			// Admin
			$admin_archive = get_field('acs_dpt_admin_archive', $post_id);
			$admin_posts_per_page = (int) get_field('acs_dpt_admin_posts_per_page', $post_id);
			$admin_orderby = get_field('acs_dpt_admin_orderby', $post_id);
			$admin_order = get_field('acs_dpt_admin_order', $post_id);
			...
			$args = array(
				...
				// ACS: Archive
				'acs_archive_template' => $archive_template,
				'acs_archive_ppp'      => $archive_posts_per_page,
				'acs_archive_orderby'  => $archive_orderby,
				'acs_archive_order'    => $archive_order,

				// ACS: Single
				'acs_single_template'  => $single_template,

				// ACS: Admin
				'acs_admin_archive'    => $admin_archive,
				'acs_admin_ppp'        => $admin_posts_per_page,
				'acs_admin_orderby'    => $admin_orderby,
				'acs_admin_order'      => $admin_order,
			);

			// Menu Position
			if(!acs_is_empty($menu_position))
			...
			if($rewrite && $rewrite_args_select){
				...
				$args['rewrite'] = array(
					'slug'          => $rewrite_args['acs_dpt_rewrite_slug'],
					'with_front'    => $rewrite_args['acs_dpt_rewrite_with_front'],
		...
		function save($name, $args, $post_id){
			...
			// Get ACS option
			$settings = acs_get_settings($this->settings);
			...
			// Update ACS option
			acs_update_settings($this->settings, $settings);
		...
		function import($name, $args){
			...
			if(isset($settings[$name])){
				return new WP_Error('acs_dpt_import_already_exists', __("Post type \"{$title}\" already exists. Import aborted."));
			}
			...
			$post = apply_filters("acs/post_type/import_post",                 $post, $name);
			$post = apply_filters("acs/post_type/import_post/name={$name}",    $post, $name);
			...
			if(!$post_id || is_wp_error($post_id)){
				return new WP_Error('acs_dpt_import_error', __("Something went wrong with the post type \"{$title}\". Import aborted."));
			}

			// Import Args
			$args = apply_filters("acs/post_type/import_args",                 $args, $name, $post_id);
			$args = apply_filters("acs/post_type/import_args/name={$name}",    $args, $name, $post_id);
			$args = apply_filters("acs/post_type/import_args/name={$post_id}", $args, $name, $post_id);
			...
			do_action("acs/post_type/import_fields",                  $name, $args, $post_id);
			do_action("acs/post_type/import_fields/name={$name}",     $name, $args, $post_id);
			do_action("acs/post_type/import_fields/id={$post_id}",    $name, $args, $post_id);
		...
		function import_fields($name, $args, $post_id){
			...
			// Register Args
			update_field('acs_dpt_name', $name, $post_id);
			...
			// Menu
			update_field('menu_position', acs_maybe_get($args, 'menu_position'), $post_id);
			...
			// Capability
			update_field('capability_type', acs_encode_choices($args['capability_type'], false), $post_id);
			update_field('map_meta_cap', $args['map_meta_cap'], $post_id);

			if(isset($args['capabilities']))
				update_field('capabilities', acs_encode_choices($args['capabilities'], false), $post_id);

			// Archive
			update_field('acs_dpt_archive_template', $args['acs_archive_template'], $post_id);
			update_field('acs_dpt_archive_posts_per_page', $args['acs_archive_ppp'], $post_id);
			update_field('acs_dpt_archive_orderby', $args['acs_archive_orderby'], $post_id);
			update_field('acs_dpt_archive_order', $args['acs_archive_order'], $post_id);
			update_field('has_archive', $args['has_archive'], $post_id);

			// Single
			update_field('acs_dpt_single_template', $args['acs_single_template'], $post_id);
			update_field('rewrite', $args['rewrite'], $post_id);

			// Admin
			update_field('acs_dpt_admin_posts_per_page', $args['acs_admin_ppp'], $post_id);
			update_field('acs_dpt_admin_orderby', $args['acs_admin_orderby'], $post_id);
			update_field('acs_dpt_admin_order', $args['acs_admin_order'], $post_id);
			...
			if($args['rewrite'] && is_array($args['rewrite'])){
				...
				update_field('rewrite_args_acs_dpt_rewrite_slug', $args['rewrite']['slug'], $post_id);
				update_field('rewrite_args_acs_dpt_rewrite_with_front', $args['rewrite']['with_front'], $post_id);
		...
		function export_data($name){
			...
			// Filters
			$args = apply_filters("acs/post_type/export_args",                 $args, $name);
			$args = apply_filters("acs/post_type/export_args/name={$name}",    $args, $name);
		...
		function export_php($data){
			...
			foreach($data as $post_type => $args){
				...
				if($l10n && $l10n_textdomain){
					...
					$args['label'] = acs_translate($args['label']);
					$args['description'] = acs_translate($args['description']);

					if(!empty($args['labels'])){

						foreach($args['labels'] as $key => &$label){
							$args['labels'][$key] = acs_translate($label);
						}
		...
		function reset(){
			...
			$args = apply_filters('acs/post_type/reset_args', array(
				'post_type'         => $this->post_type,
				'posts_per_page'    => -1,
				'fields'            => 'ids',
			...
			// Log
			acs_log('[ACS Extended] Reset: Post Types');
		...
		function add_local_field_group(){
			...
			acs_add_local_field_group(array(
				'key' => 'group_acs_dynamic_post_type',
				'title' => __('Dynamic Post Type', 'acs'),
				...
				'fields' => array(
					...
					array(
						'key' => 'field_acs_dpt_hierarchical',
						'label' => 'Hierarchical',
						'name' => 'hierarchical',
						'type' => 'true_false',
					...
					array(
						'key' => 'field_acs_dpt_exclude_from_search',
					...
					array(
						'key' => 'field_acs_dpt_publicly_queryable',
					...
					array(
						'key' => 'field_acs_dpt_delete_with_user',
					...
					array(
						'key' => 'field_acs_dpt_menu_position',
						'label' => 'Menu position',
						'name' => 'menu_position',
						'type' => 'number',
					...
					array(
						'key' => 'field_acs_dpt_show_in_menu',
						'label' => 'Show in menu',
						'name' => 'show_in_menu',
						'type' => 'true_false',
					...
					array(
						'key' => 'field_acs_dpt_show_in_menu_text',
						...
						'conditional_logic' => array(
							array(
								array(
									'field'     => 'field_acs_dpt_show_in_menu',
									'operator'  => '==',
									'value'     => '1',
								),
							),
					...
					array(
						'key' => 'field_acs_dpt_show_in_nav_menus',
					...
					array(
						'key' => 'field_acs_dpt_show_in_admin_bar',
					...
					array(
						'key' => 'field_acs_dpt_archive_template',
						'label' => 'Template',
						'name' => 'acs_dpt_archive_template',
						'type' => 'text',
						'instructions' => 'ACS Extended: Which template file to load for the archive query. More informations on <a href="https://developer.wordpress.org/themes/basics/template-hierarchy/">Template hierarchy</a>',
						'required' => 0,
						'conditional_logic' => 0,
						'wrapper' => array(
							'width' => '',
						...
						'prepend' => trailingslashit(acs_get_setting('theme_folder')),
						'append' => '',
						'maxlength' => '',
					),
					array(
						'key' => 'field_acs_dpt_has_archive',
						'label' => 'Has archive',
						'name' => 'has_archive',
						'type' => 'true_false',
					...
					array(
						'key' => 'field_acs_dpt_has_archive_slug',
						'label' => 'Slug',
						...
						'conditional_logic' => array(
							array(
								array(
									'field' => 'field_acs_dpt_has_archive',
									'operator' => '==',
									'value' => '1',
								),
							),
					...
					array(
						'key' => 'field_acs_dpt_archive_posts_per_page',
						'label' => 'Posts per page',
						'name' => 'acs_dpt_archive_posts_per_page',
						'type' => 'number',
						'instructions' => 'ACS Extended: Number of posts to display in the archive page',
						'required' => 0,
						'wrapper' => array(
							'width' => '',
							'class' => '',
					...
					array(
						'key' => 'field_acs_dpt_archive_orderby',
						'label' => 'Order by',
						'name' => 'acs_dpt_archive_orderby',
						'type' => 'text',
						'instructions' => 'ACS Extended: Sort retrieved posts by parameter in the archive page. Defaults to \'date (post_date)\'.',
						'required' => 0,
						'wrapper' => array(
							'width' => '',
							'class' => '',
						...
						'acs_update' => array(
							'5c9479dec93c4' => array(
								'acs_update_function' => 'sanitize_title',
							),
						),
						'acs_permissions' => '',
						'default_value' => 'date',
					...
					array(
						'key' => 'field_acs_dpt_archive_order',
						'label' => 'Order',
						'name' => 'acs_dpt_archive_order',
						'type' => 'select',
						'instructions' => 'ACS Extended: Designates the ascending or descending order of the \'orderby\' parameter in the archive page. Defaults to \'DESC\'.',
						'required' => 0,
						'wrapper' => array(
							'width' => '',
							'class' => '',
					...
					array(
						'key' => 'field_acs_dpt_single_template',
						'label' => 'Template',
						'name' => 'acs_dpt_single_template',
						'type' => 'text',
						'instructions' => 'ACS Extended: Which template file to load for the single query. More informations on <a href="https://developer.wordpress.org/themes/basics/template-hierarchy/">Template hierarchy</a>',
						'required' => 0,
						'conditional_logic' => 0,
						'wrapper' => array(
							'width' => '',
					...
					array(
						'key' => 'field_acs_dpt_rewrite_args_select',
					...
					array(
						'key' => 'field_acs_dpt_rewrite_args',
						'label' => 'Rewrite Arguments',
						'name' => 'rewrite_args',
						'type' => 'group',
						...
						'conditional_logic' => array(
							array(
								...
								array(
									'field' => 'field_acs_dpt_rewrite_args_select',
									'operator' => '==',
									'value' => '1',
								),
							),
						...
						'sub_fields' => array(
							array(
								'key' => 'field_acs_dpt_rewrite_slug',
								'label' => 'Slug',
								'name' => 'acs_dpt_rewrite_slug',
								'type' => 'text',
							...
							array(
								'key' => 'field_acs_dpt_rewrite_with_front',
								'label' => 'With front',
								'name' => 'acs_dpt_rewrite_with_front',
								'type' => 'true_false',
							...
							array(
								'key' => 'field_acs_dpt_rewrite_feeds',
								'label' => 'Feeds',
								'name' => 'feeds',
								'type' => 'true_false',
							...
							array(
								'key' => 'field_acs_dpt_rewrite_pages',
								'label' => 'Pages',
								'name' => 'pages',
								'type' => 'true_false',
					...
					array(
						'key' => 'field_acs_dpt_admin_archive',
						'label' => 'Archive Page',
						'name' => 'acs_dpt_admin_archive',
						'type' => 'true_false',
						'instructions' => 'ACS Extended: Add an "Archive" Options Page as submenu of the post type.',
						'required' => 0,
						'wrapper' => array(
							'width' => '',
							'class' => '',
					...
					array(
						'key' => 'field_acs_dpt_admin_posts_per_page',
						'label' => 'Posts per page',
						'name' => 'acs_dpt_admin_posts_per_page',
						'type' => 'number',
						'instructions' => 'ACS Extended: Number of posts to display on the admin list screen.',
						'required' => 0,
						'conditional_logic' => 0,
						'wrapper' => array(
							'width' => '',
					...
					array(
						'key' => 'field_acs_dpt_admin_orderby',
						'label' => 'Order by',
						'name' => 'acs_dpt_admin_orderby',
						'type' => 'text',
						'instructions' => 'ACS Extended: Sort retrieved posts by parameter in the admin list screen. Defaults to \'date (post_date)\'.',
						'required' => 0,
						'conditional_logic' => 0,
						'wrapper' => array(
							'width' => '',
					...
					array(
						'key' => 'field_acs_dpt_admin_order',
						'label' => 'Order',
						'name' => 'acs_dpt_admin_order',
						'type' => 'select',
						'instructions' => 'ACS Extended: Designates the ascending or descending order of the \'orderby\' parameter in the admin list screen. Defaults to \'DESC\'.',
						'required' => 0,
						'conditional_logic' => 0,
						'wrapper' => array(
							'width' => '',
...
Default: if empty, \'name\' is set to value of \'label\', and \'singular_name\' is set to value of \'name\'.',
						...
						'sub_fields' => array(
							array(
								'key' => 'field_acs_dpt_singular_name',
								'label' => 'Singular name',
								'name' => 'singular_name',
								'type' => 'text',
								'instructions' => '',
							...
							array(
								'key' => 'field_acs_dpt_add_new_item',
								'label' => 'Add new item',
								'name' => 'add_new_item',
								'type' => 'text',
								'instructions' => '',
							...
							array(
								'key' => 'field_acs_dpt_search_items',
								'label' => 'Search items',
								'name' => 'search_items',
								'type' => 'text',
								'instructions' => '',
							...
							array(
								'key' => 'field_acs_dpt_not_found_in_trash',
							...
							array(
								'key' => 'field_acs_dpt_parent_item_colon',
							...
							array(
								'key' => 'field_acs_dpt_insert_into_item',
							...
							array(
								'key' => 'field_acs_dpt_uploaded_to_this_item',
							...
							array(
								'key' => 'field_acs_dpt_featured_image',
							...
							array(
								'key' => 'field_acs_dpt_set_featured_image',
							...
							array(
								'key' => 'field_acs_dpt_remove_featured_image',
							...
							array(
								'key' => 'field_acs_dpt_use_featured_image',
							...
							array(
								'key' => 'field_acs_dpt_filter_items_list',
							...
							array(
								'key' => 'field_acs_dpt_items_list_navigation',
							...
							array(
								'key' => 'field_acs_dpt_name_admin_bar',
							...
							array(
								'key' => 'field_acs_dpt_item_published',
							...
							array(
								'key' => 'field_acs_dpt_item_published_privately',
							...
							array(
								'key' => 'field_acs_dpt_item_reverted_to_draft',
							...
							array(
								'key' => 'field_acs_dpt_item_scheduled',
							...
							array(
								'key' => 'field_acs_dpt_item_updated',
								'label' => 'Item updated',
								'name' => 'item_updated',
								'type' => 'text',
								'instructions' => '',
					...
					array(
						'key' => 'field_acs_dpt_tab_capability',
						'label' => 'Capability',
						'name' => '',
						'type' => 'tab',
						'instructions' => '',
					...
					array(
						'key' => 'field_acs_dpt_capability_type',
...
stories',
					...
					array(
						'key' => 'field_acs_dpt_capabilities',
						'label' => 'Capabilities',
						'name' => 'capabilities',
						'type' => 'textarea',
...
etc...',
					...
					array(
						'key' => 'field_acs_dpt_map_meta_cap',
						'label' => 'Map meta cap',
						'name' => 'map_meta_cap',
						'type' => 'select',
						'instructions' => '',
					...
					array(
						'key' => 'field_acs_dpt_show_in_rest',
						'label' => 'Show in rest',
						'name' => 'show_in_rest',
						'type' => 'true_false',
					...
					array(
						'key' => 'field_acs_dpt_rest_controller_class',
	...
	acs_new_instance('acs_dynamic_post_types');
